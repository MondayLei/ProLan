...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(aunit? e) e]
        [(closure? e) e]
        ;; Miss a () here and made a terrible misstake, IT'S SO HARD TO DISCOVER!!!
        [(fun? e) (closure env e)]
        ;;
        [(ifgreater? e)
        ...
        [(mlet? e)
         (let ([v (eval-under-env (mlet-e e) env)]
               [exp (mlet-var e)])
           (eval-under-env (mlet-body e) (cons (cons exp v) env)))]
        ;;
        [(call? e)
         (let ([cls (eval-under-env (call-funexp e) env)]
           ...
           (if (not (closure? cls)) 
               ...
               (letrec ([func (closure-fun cls)]
                        [fun_name (fun-nameopt func)]
                        [call_env (cons (cons (fun-formal func) arg)
                                        (closure-env cls))])
                 (eval-under-env (fun-body func)
                                 (if (string? fun_name)
                                     (cons (cons fun_name cls) call_env) call_env)))))]
        ;;
        [(apair? e)
...
(define mupl-map
  (fun "func" "para" 
       (fun "f" "list" 
            (ifaunit(var "list")
                    (aunit)
                    (apair (call (var "para") (fst (var "list"))) 
                           (call (var "f") (snd (var "list"))))))))
